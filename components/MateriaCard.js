...
import { MapPin, Users, GraduationCap } from "lucide-react"; // Añadimos GraduationCap opcional
...
export default function MateriaCard({ materia, amigosAca = [] }) {
  ...
  return (
    ...
    >
      <div>
        <h3 className="text-xs font-black leading-tight line-clamp-2 uppercase italic tracking-tighter text-white">
          {materia.nombre}
        </h3>
        {/* VISUALIZACIÓN DEL PROFESOR EN TU TARJETA */}
        {materia.profesor && (
          <p className="text-[8px] font-bold text-white/50 uppercase mt-1 truncate tracking-wider">
            {materia.profesor}
          </p>
        )}
      </div>
        ...
        {amigosAca.map((amigo, idx) => {
          ...
          return (
            <div key={amigo.id} className="relative group/tooltip">
              ...
              {/* TOOLTIP EMERGENTE ACTUALIZADO CON PROFESOR */}
              <div className="absolute bottom-full right-0 mb-3 w-48 p-3 bg-card-bg border border-white/10 rounded-[1.5rem] shadow-[0_10px_40px_rgba(0,0,0,0.6)] opacity-0 scale-90 pointer-events-none group-hover/tooltip:opacity-100 group-hover/tooltip:scale-100 transition-all z-[110] backdrop-blur-md">
                ...
                {/* MOSTRAR PROFESOR DEL AMIGO EN EL TOOLTIP */}
                {suMateria?.profesor && (
